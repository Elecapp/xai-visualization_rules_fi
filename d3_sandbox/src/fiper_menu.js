...
function FiperClassificationBox() {
  ...
  function me(selection) {
    ...
    selection.selectAll('text.classification')
      ...
      .attr('dy', '1em')
      .attr('fill', FTTemplate.TEXT_COLOR)
      .html(d => text2tspan(`_*Classification:*_ Predicted class is _*${d.predicted_class}*_ with a probability of _*${formatValue(d.predicted_proba[d.predicted_class])}*_`, 42, GUTTER));
...
function FiperMenuPaletteSelector() {
  ...
  let currentSelection = paletteOptions[0].value;
  ...
  function me(selection) {
    ...
    selection.selectAll('rect.palette')
      ...
      .attr('stroke', FTTemplate.TEXT_COLOR)
      .attr('stroke-width', d => d.value === currentSelection ? 0.5 : 0)
      .attr('cursor', 'pointer')
      .on('click', (d) => {
        const selectedPalette = d3.select(d.target).datum();
        FTTemplate = colorSet[selectedPalette.value];
        currentSelection = selectedPalette.value;
